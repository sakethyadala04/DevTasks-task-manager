...
const TaskModal = ({ isOpen, taskToEdit, onClose, onLogout, onSave }) => {
  ...
  const handleSubmit = useCallback(
    ...
    [taskData, getHeaders, onClose, onLogout, onSave, today]
  );
    ...
    <div className="fixed inset-0 backdrop-blur-sm bg-black/20 z-50 flex items-center justify-center p-4">
      <div className="bg-white border border-purple-100 rounded-xl max-w-md w-full shadow-lg p-6 animate-fadeIn">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">
              <AlignLeft className="inline w-4 h-4 mr-1 text-purple-500" />
              Description
            </label>
            <textarea
              name="description"
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">
                <Flag className="inline w-4 h-4 mr-1 text-purple-500" />
                Priority
              </label>
              <select
                name="priority"
